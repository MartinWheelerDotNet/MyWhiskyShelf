...
public class ValidatePagingQueryInRangeFilter : IEndpointFilter
{
    public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next)
    {
        ...
        if (errors.Count > 0)
            return ValidationProblemResults.InvalidPagingParameters(errors);

        return await next(context);
    }

    private static Dictionary<string, string[]> ValidatePagingParameters(bool hasPage, bool hasAmount, int page,
        int amount)
    {
